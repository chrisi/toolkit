...
func TestTools_RandomString(t *testing.T) {
	...
	if len(s) != 10 {
		t.Error("wrong length randem string returned")
	}
}
...
var uploadTests = []struct {
	...
	{name: "not allowed", allowedTypes: []string{"image/jpeg", "image/png"}, renameFile: false, errorExpected: true},
}
...
func TestTools_UploadFiles(t *testing.T) {
	for _, e := range uploadTests {
		...
		go func() {
			...
			part, err := writer.CreateFormFile("file", "./testdata/hund.png")
			if err != nil {
				t.Error(err)
			}
			f, err := os.Open("./testdata/hund.png")
			if err != nil {
				t.Error(err)
			}
			defer f.Close()
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	testTools.DownloadStaticFile(rr, req, "./testdata", "blume.jpg", "flower.jpg")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "10411" {
		t.Error("wrong content length of", res.Header["Content-Length"][0])
	}

	if res.Header["Content-Disposition"][0] != "attachment; filename=\"flower.jpg\"" {
...
var jsonTests = []struct {
	...
	{name: "unknown field", json: `{"bar": "1"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allow unknown field", json: `{"bar": "1"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		if err != nil {
			t.Logf("%s: error creating request: %s", e.name, err.Error())
		}
		...
		if e.errorExpected && err == nil {
			t.Errorf("%s: expected error but received none\n", e.name)
		}
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if !payload.Error {
		t.Errorf("expected error flag to be true, got false")
	}
